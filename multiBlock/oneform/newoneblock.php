...
$fileDelete = $folder . 'hideOneDelete.txt';
$fileAddNew = $folder . 'hideOneAddNew.txt';
...
<div class="mb_catlist">
	...
	if (isset($_GET['newmulticategory']) || isset($_GET['refresh'])) {
		...
		foreach (glob(GSDATAOTHERPATH . "/oneBlock/" . str_replace(" ", "-", $_GET['newmulticategory']) . "/*.json") as $filename) {
			global $SITEURL;
			$info = pathinfo($filename);
			$namevalue = str_replace(' ', '-', basename($filename, '.' . $info['extension']));
			$name = str_replace('-', ' ', basename($filename, '.' . $info['extension']));
			...
			echo '<li data-id="' . $counterlist . '"><p>' . $content->nametitle . '<span style="font-size:12px;opacity:0.6;margin-left:10px;"> slug: ' . $name . '</span> </p>
				...
				<a href="' . $SITEURL . 'admin/load.php?id=multiBlock&newblock1&newmulticategory=' . $_GET['newmulticategory'] . '&namefile=' . $name . '&nametitle=' . $content->nametitle . '" class="mb_edit" style="width:45px;">' . i18n_r("multiBlock/EDIT") . '</a>

				<form method="post"  onclick="return confirm(`'.i18n_r('multiBlock/DELETEQUESTION').'`);" 
				style="display:inline-flex" 
				action="' . $SITEURL . 'admin/load.php?id=multiBlock&newoneblock&newmulticategory=' . @$_GET['newmulticategory'] . '&refresh">
	...
	if (isset($_GET['newmulticategory'])) {
		global $SITEURL;
		echo '
			<form method="post" style="margin-top:20px;" action="' . $SITEURL . 'admin/load.php?id=multiBlock&newoneblock&newmulticategory=' . $_GET['newmulticategory'] . '">
...
<script>
	document.querySelector('.cats').setAttribute('action', '<?php global $SITEURL;
															echo $SITEURL; ?>admin/load.php?id=multiBlock&newoneblock&newmulticategory=' + document.querySelector('.mb_category').value);

	if (document.querySelector('.addnewblock') !== null) {
		document.querySelector('.addnewblock').setAttribute('href', '<?php global $SITEURL;
																		echo $SITEURL; ?>admin/load.php?id=multiBlock&newblock1&newmulticategory=' + document.querySelector('.mb_category').value);
	}
	...
	if (isset($_POST['choosecat'])) {
		...
		echo "document.querySelector('.addnewblock').setAttribute('href','" . $SITEURL . "admin/load.php?id=multiBlock&newblock1&newmulticategory=" . @$_GET['newmulticategory'] . "')";
	}
...
if (isset($_POST['delthisbtn'])) {
	unlink(GSDATAOTHERPATH . "oneBlock/" . str_replace(" ", "-", $_GET['newmulticategory']) . "/" . $_POST['delthis'] . ".json");
	echo ("<meta http-equiv='refresh' content='0'>");
}
